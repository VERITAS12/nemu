...
void dram_read_64(hwaddr_t addr, void *data){
	Assert(addr < HW_MEM_SIZE, "physical address %x is outside of the physical memory!", addr);
	...
	/* burst read */
	memcpy(data, rowbufs[rank][bank].buf + col, CACHE_LEN);
}

void dram_write_64(hwaddr_t addr, void *data){
	Assert(addr < HW_MEM_SIZE, "physical address %x is outside of the physical memory!", addr);
	...
	/* burst write */
	memcpy(rowbufs[rank][bank].buf + col, data, CACHE_LEN);
